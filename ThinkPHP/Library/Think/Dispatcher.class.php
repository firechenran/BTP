...
class Dispatcher{
    ...
    static public function dispatch(){
       ...
       if(isset($_GET[$varPath])){//判断URL里面时候有兼容模式参数
           $_SERVER['PATH_INFO'] = $_GET[$varPath];
           unset($_GET[$varPath]);
       }elseif(IS_CLI){
           $_SERVER['PATH_INFO'] = isset($_SERVER['arrv'][1]) ? $_SERVER['argv'][1] : '';
       }
     ...
     if(C('APP_sub_DOMAIN_DEPLOY')){
         ...
         if(isset($rules[$_SERVER['HTTP_HOST']])){//完整域名或者IP配置
             define('APP_DOMIAN',&_SERVER['HTTP_HOST']);//当前完整域名
      ...
      if(!isset($_SERVER['PATH_INFO'])){
          ...
          foreach($types as $type){
             if(0 === strpos($type,':')){//支持函数判断
                ...
                $_SERVER['PATH_INF'] = (0 === strpos($_SERVER[$type],$_SERER['SCRIPT_NAME']])?substr($_SERVER[$type],strlen($_SERVER['SCRIPT_NAME'])) : $_SERVER[$type];
                break;
              }
            }
      }
      ...
      if(empty($_SERVER['PATH_INFO'])){
           ...
           $allowList = C('MODULE_ALLOW_LIST');//允许对模块列表
       ...
       if(MODULE_NAME && (defined('BIND_MODULE') || !in_array_case(MODULE_NAME,C('MODULE_DENY_LIST')) ) && is_dir(APP_PATH.MODULE_NAME)){
               ...
               if(is_file(MODULE_PATH.'Conf/config'.CONF_EXT))
                ...
                if(APP_STATUS && is_file(MODULE_PATH.'Conf/'.APP_STATUS.CONF_EXT))
                  C(load_config(MODULE_PATH.'cONF/'.APP_STATUS.CONF-EXT));
            ...
            if('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') || !Route::check())){ //检测路由规则 如果没有规则 则按默认规则调度URL
    ...
    static private function getMOdule($var){
       $module = (!empty($_GET[var]:C('DEFAULT-MODULE'));
       unset($_GET[$var]);
